...
public class TeamRecordTests {
    private static final String TEAM_RECORDS = "teams-records.json"; //TODO:Change this
    ...
    public void testTeamDeserialization() throws IOException {
        ...
        for (var i = 0; i < teamRecords.size(); i++) {
            Assert.assertNotNull(teamRecords.get(i).getAwayRecord());
            Assert.assertNotNull(teamRecords.get(i).getDivisionRecord());
            Assert.assertNotNull(teamRecords.get(i).getHomeRecord());
            Assert.assertNotNull(teamRecords.get(i).getOverallRecord());
        }
    }
}